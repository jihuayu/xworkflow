...
use xworkflow::sandbox::{CodeLanguage, ExecutionConfig, SandboxManager, SandboxManagerConfig, SandboxRequest};
...
fn bench_js_sandbox(c: &mut Criterion) {
    ...
    let obj_transform = "function main(inputs) { return { out: {a: inputs.a, b: inputs.b, c: inputs.c} }; }".to_string();
    ...
    let cases = vec![
        ...
        ("js_json_parse_stringify", json_ops, serde_json::json!({"s": "{\"a\":1}"})),
        ("js_array_100", array_100, input_small.clone()),
        ("js_array_1000", array_1000, input_large.clone()),
        ("js_object_transform_20_fields", obj_transform, input_small.clone()),
        ("js_builtins_sha256", sha256, serde_json::json!({"s": "hello"})),
        ("js_builtins_uuid", uuid, serde_json::json!({})),
        ("js_builtins_base64", base64, serde_json::json!({"s": "hello"})),
    ];
    ...
    c.bench_function("js_validation_only", |b| {
        b.to_async(&rt).iter(|| async {
            let _ = manager.validate(&validate_only, CodeLanguage::JavaScript).await;
        });
    });
}
