...
use xworkflow_types::sandbox::SandboxError;
...
impl AstCodeAnalyzer {
    fn analyze_script(&self, script: &Script, interner: &Interner) -> CodeAnalysisResult {
...
impl<'a> AstSecurityVisitor<'a> {
    ...
    fn ident_name(&self, ident: Identifier) -> Option<String> {
        self.interner.resolve(ident.sym()).map(|s| s.to_string())
    }
