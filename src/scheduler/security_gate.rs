...
impl SchedulerSecurityGate for NoopSchedulerSecurityGate {
    ...
    async fn audit_validation_failed(
        &self,
        _context: &WorkflowContext,
    ...
    impl SchedulerSecurityGate for RealSchedulerSecurityGate {
        ...
        async fn on_workflow_start(
            ...
            if let (Some(governor), Some(group)) =
                (context.resource_governor(), context.resource_group())
            {
                governor
...
mod tests {
    ...
    async fn test_real_scheduler_security_gate_on_workflow_start_with_governor() {
        ...
        gate.record_workflow_end(&context, workflow_id.as_deref())
            .await;
    }
