...
fn apply_selector_limits(
    ...
    if depth > cfg.max_depth {
        diags.push(types::Diagnostic {
            level: types::DiagnosticLevel::Error,
            code: "E420".to_string(),
            message: format!("Selector depth exceeds max {}: {}", cfg.max_depth, depth),
    ...
    if length > cfg.max_length {
        diags.push(types::Diagnostic {
            level: types::DiagnosticLevel::Error,
            code: "E421".to_string(),
            message: format!("Selector length exceeds max {}: {}", cfg.max_length, length),
            node_id: Some(node_id.to_string()),
            edge_id: None,
            field_path: Some(field_path.to_string()),
        });
...
mod tests {
    ...
    fn test_end_empty_value_selector() {
        ...
        assert!(
            report
                .diagnostics
                .iter()
                .any(|d| d.code == "E217" || d.code == "E202" || d.code == "E401"),
            "got: {:?}",
            report.diagnostics
        );
    }
    ...
    fn test_if_else_empty_conditions() {
        ...
        let report = validate_schema(&schema);
        assert!(
            report.diagnostics.iter().any(|d| d.code == "E205"),
            "got: {:?}",
            report.diagnostics
        );
    }
    ...
    fn test_if_else_duplicate_case_id() {
        ...
        let report = validate_schema(&schema);
        assert!(
            report.diagnostics.iter().any(|d| d.code == "E206"),
            "got: {:?}",
            report.diagnostics
        );
    }
    ...
    fn test_iteration_empty_sub_graph() {
        ...
        let report = validate_schema(&schema);
        assert!(
            report.diagnostics.iter().any(|d| d.code == "E211"),
            "got: {:?}",
            report.diagnostics
        );
    }
    ...
    fn test_iteration_empty_selector() {
        ...
        let report = validate_schema(&schema);
        assert!(
            report.diagnostics.iter().any(|d| d.code == "E212"),
            "got: {:?}",
            report.diagnostics
        );
    }
    ...
    fn test_loop_empty_sub_graph() {
        ...
        let report = validate_schema(&schema);
        assert!(
            report.diagnostics.iter().any(|d| d.code == "E213"),
            "got: {:?}",
            report.diagnostics
        );
    }
    ...
    fn test_loop_missing_break_condition() {
        ...
        let report = validate_schema(&schema);
        assert!(
            report.diagnostics.iter().any(|d| d.code == "E214"),
            "got: {:?}",
            report.diagnostics
        );
    }
    ...
    fn test_list_operator_missing_operation() {
        ...
        let report = validate_schema(&schema);
        assert!(
            report.diagnostics.iter().any(|d| d.code == "E216"),
            "got: {:?}",
            report.diagnostics
        );
    }
    ...
    fn test_branch_case_missing_edge() {
        ...
        let report = validate_schema(&schema);
        assert!(
            report.diagnostics.iter().any(|d| d.code == "E302"),
            "got: {:?}",
            report.diagnostics
        );
    }
    ...
    fn test_branch_unknown_source_handle() {
        ...
        let report = validate_schema(&schema);
        assert!(
            report.diagnostics.iter().any(|d| d.code == "E304"),
            "got: {:?}",
            report.diagnostics
        );
    }
    ...
    #[test]
    fn test_question_classifier_missing_default_edge() {
        let json = r#"{"version":"0.1.0","nodes":[{"id":"start","data":{"type":"start","title":"S"}},{"id":"qc","data":{"type":"question-classifier","title":"QC","query_variable_selector":["start","query"],"categories":[{"category_id":"cat_support","category_name":"Support"}],"model":{"provider":"openai","name":"gpt-4o-mini"}}},{"id":"a","data":{"type":"end","title":"A","outputs":[]}}],"edges":[{"source":"start","target":"qc"},{"source":"qc","target":"a","source_handle":"cat_support"}]}"#;
        let schema: WorkflowSchema = serde_json::from_str(json).unwrap();
        let report = validate_schema(&schema);
        assert!(
            report.diagnostics.iter().any(|d| d.code == "E301"),
            "got: {:?}",
            report.diagnostics
        );
    }
    ...
    #[test]
    fn test_question_classifier_missing_category_edge() {
        let json = r#"{"version":"0.1.0","nodes":[{"id":"start","data":{"type":"start","title":"S"}},{"id":"qc","data":{"type":"question-classifier","title":"QC","query_variable_selector":["start","query"],"categories":[{"category_id":"cat_support","category_name":"Support"},{"category_id":"cat_sales","category_name":"Sales"}],"model":{"provider":"openai","name":"gpt-4o-mini"}}},{"id":"a","data":{"type":"end","title":"A","outputs":[]}},{"id":"d","data":{"type":"end","title":"D","outputs":[]}}],"edges":[{"source":"start","target":"qc"},{"source":"qc","target":"a","source_handle":"cat_support"},{"source":"qc","target":"d","source_handle":"default"}]}"#;
        let schema: WorkflowSchema = serde_json::from_str(json).unwrap();
        let report = validate_schema(&schema);
        assert!(
            report.diagnostics.iter().any(|d| d.code == "E302"),
    ...
    #[test]
    fn test_question_classifier_unknown_source_handle() {
        let json = r#"{"version":"0.1.0","nodes":[{"id":"start","data":{"type":"start","title":"S"}},{"id":"qc","data":{"type":"question-classifier","title":"QC","query_variable_selector":["start","query"],"categories":[{"category_id":"cat_support","category_name":"Support"}],"model":{"provider":"openai","name":"gpt-4o-mini"}}},{"id":"a","data":{"type":"end","title":"A","outputs":[]}},{"id":"d","data":{"type":"end","title":"D","outputs":[]}},{"id":"x","data":{"type":"end","title":"X","outputs":[]}}],"edges":[{"source":"start","target":"qc"},{"source":"qc","target":"a","source_handle":"cat_support"},{"source":"qc","target":"d","source_handle":"default"},{"source":"qc","target":"x","source_handle":"unknown"}]}"#;
        let schema: WorkflowSchema = serde_json::from_str(json).unwrap();
        let report = validate_schema(&schema);
        assert!(
            report.diagnostics.iter().any(|d| d.code == "E304"),
    ...
    #[test]
    fn test_question_classifier_duplicate_category_id() {
        let json = r#"{"version":"0.1.0","nodes":[{"id":"start","data":{"type":"start","title":"S"}},{"id":"qc","data":{"type":"question-classifier","title":"QC","query_variable_selector":["start","query"],"categories":[{"category_id":"cat_support","category_name":"Support"},{"category_id":"cat_support","category_name":"Support2"}],"model":{"provider":"openai","name":"gpt-4o-mini"}}},{"id":"a","data":{"type":"end","title":"A","outputs":[]}},{"id":"d","data":{"type":"end","title":"D","outputs":[]}}],"edges":[{"source":"start","target":"qc"},{"source":"qc","target":"a","source_handle":"cat_support"},{"source":"qc","target":"d","source_handle":"default"}]}"#;
        let schema: WorkflowSchema = serde_json::from_str(json).unwrap();
        let report = validate_schema(&schema);
        assert!(
            report.diagnostics.iter().any(|d| d.code == "E222"),
            "got: {:?}",
            report.diagnostics
        );
    }
    ...
    fn test_selector_downstream_reference() {
        ...
        let report = validate_schema(&schema);
        assert!(
            report.diagnostics.iter().any(|d| d.code == "E403"),
            "got: {:?}",
            report.diagnostics
        );
    }
    ...
    fn test_dify_template_unknown_node_ref() {
        ...
        let report = validate_schema(&schema);
        assert!(
            report.diagnostics.iter().any(|d| d.code == "W502"),
            "got: {:?}",
            report.diagnostics
        );
    }
    ...
    fn test_start_invalid_variable_type() {
        ...
        let report = validate_schema(&schema);
        assert!(
            report.diagnostics.iter().any(|d| d.code == "E201"),
            "got: {:?}",
            report.diagnostics
        );
    }
    ...
    fn test_assigner_bad_config() {
        ...
        let report = validate_schema(&schema);
        assert!(
            report.diagnostics.iter().any(|d| d.code == "E217"),
            "got: {:?}",
            report.diagnostics
        );
    }
    ...
    fn test_stub_node_type_warning() {
        let json = r#"{"version":"0.1.0","nodes":[{"id":"start","data":{"type":"start","title":"S"}},{"id":"kr","data":{"type":"knowledge-retrieval","title":"KR"}},{"id":"end","data":{"type":"end","title":"E","outputs":[]}}],"edges":[{"source":"start","target":"kr"},{"source":"kr","target":"end"}]}"#;
        let schema: WorkflowSchema = serde_json::from_str(json).unwrap();
        let report = validate_schema(&schema);
        assert!(
            report.diagnostics.iter().any(|d| d.code == "W202"),
            "got: {:?}",
            report.diagnostics
        );
    }
    ...
    fn test_validate_with_config_node_id_too_long() {
        ...
        let json = format!(
            r#"{{"version":"0.1.0","nodes":[{{"id":"start","data":{{"type":"start","title":"S"}}}},{{"id":"{}","data":{{"type":"code","title":"C","code":"x","language":"javascript"}}}},{{"id":"end","data":{{"type":"end","title":"E","outputs":[]}}}}],"edges":[{{"source":"start","target":"{}"}},{{"source":"{}","target":"end"}}]}}"#,
    ...
    fn test_validate_with_config_selector_limits() {
        ...
        assert!(report
            .diagnostics
            .iter()
            .any(|d| d.code == "E420" || d.code == "E421" || d.code == "E422"));
    }
    ...
    fn test_jinja_template_syntax_error_e501() {
        ...
        let report = validate_schema(&schema);
        assert!(
            report.diagnostics.iter().any(|d| d.code == "E501"),
            "got: {:?}",
            report.diagnostics
        );
    }
    ...
    fn test_branch_extra_source_edge_w301() {
        ...
        let report = validate_schema(&schema);
        assert!(
            report.diagnostics.iter().any(|d| d.code == "W301"),
            "got: {:?}",
            report.diagnostics
        );
    }
    ...
    fn test_template_selector_unknown_ref() {
        ...
        let report = validate_schema(&schema);
        assert!(
            report.diagnostics.iter().any(|d| d.code == "E402"),
            "got: {:?}",
            report.diagnostics
        );
    }
    ...
    fn test_fail_branch_allows_source_handle() {
        ...
        let report = validate_schema(&schema);
        assert!(
            !report.diagnostics.iter().any(|d| d.code == "E303"),
            "got: {:?}",
            report.diagnostics
        );
    }
    ...
    fn test_http_dify_template_headers_params_w502() {
        ...
        let schema: WorkflowSchema = serde_json::from_str(json).unwrap();
        let report = validate_schema(&schema);
        let w502_count = report
            .diagnostics
            .iter()
            .filter(|d| d.code == "W502")
            .count();
        assert!(
            w502_count >= 2,
            "expected >=2 W502, got: {:?}",
    ...
    fn test_multiple_errors_in_single_workflow() {
        ...
        assert!(
            report.diagnostics.iter().any(|d| d.code == "E207"),
            "missing E207: {:?}",
            report.diagnostics
        );
        assert!(
            report.diagnostics.iter().any(|d| d.code == "E208"),
            "missing E208: {:?}",
            report.diagnostics
        );
        assert!(
            report.diagnostics.iter().any(|d| d.code == "E209"),
            "missing E209: {:?}",
            report.diagnostics
        );
    }
