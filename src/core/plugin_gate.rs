...
mod real {
  ...
  impl RealPluginGate {
    ...
    async fn execute_hooks<F>(&self, hook_point: HookPoint, payload_fn: F) -> WorkflowResult<Vec<Value>>
    where
      F: FnOnce() -> Value,
    {
  ...
  pub fn new_gate(
    ...
    Arc::new(RealPluginGate::new(variable_pool, event_emitter, plugin_registry))
  }
}
