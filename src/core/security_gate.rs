        ...
        async fn handle_quota_error(&self, node_id: &str, err: QuotaError) -> NodeError {
            let (quota_type, limit, current, message) = match &err {
                ...
                QuotaError::VariablePoolTooLarge {
                    ...
                    *current as u64,
                    format!(
                        "Variable pool entries exceeded: {}/{}",
                        current, max_entries
                    ),
                ),
...
mod tests {
    ...
    async fn test_noop_security_gate_check_before_node() {
        ...
        let result = gate
            .check_before_node("n1", "code", &serde_json::json!({}))
            .await;
        assert!(result.is_ok());
    }
    ...
    async fn test_real_security_gate_check_before_http_no_governor() {
        ...
        let gate = new_security_gate(context, pool);
        let result = gate
            .check_before_node("n1", "http-request", &serde_json::json!({}))
            .await;
        assert!(result.is_ok());
    }
    ...
    async fn test_real_security_gate_with_governor_http_rate_ok() {
        ...
        let gate = new_security_gate(context, pool);
        let result = gate
            .check_before_node("h1", "http-request", &serde_json::json!({}))
            .await;
        assert!(result.is_ok());
    }
    ...
    async fn test_real_security_gate_enforce_output_with_policy_under_limit() {
        ...
        use crate::security::policy::{NodeResourceLimits, SecurityLevel, SecurityPolicy};
        ...
        ready.insert(
            "result".to_string(),
            crate::core::variable_pool::Segment::String("small output".to_string()),
        );
    ...
    async fn test_real_security_gate_enforce_output_too_large() {
        ...
        ready.insert(
            "result".to_string(),
            crate::core::variable_pool::Segment::String(
                "this is a long output that exceeds 10 bytes".to_string(),
            ),
        );
        let outputs = NodeOutputs::Sync(ready);
        let result = NodeRunResult {
        ...
        assert!(
            err.to_string().contains("too large")
                || err.to_string().contains("OutputTooLarge")
